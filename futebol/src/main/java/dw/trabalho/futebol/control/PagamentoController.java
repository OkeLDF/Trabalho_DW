...
public class PagamentoController {
    ...
    @GetMapping("/jogadorId/{id}")
    public ResponseEntity<List<Pagamento>> getPagamentoByJogadorId(@PathVariable("id")Long id){
        Optional<Jogador> jogador = jogadorRep.findById(id);
        List<Pagamento> pagamentos = rep.findByJogador(jogador.get());
        if(!pagamentos.isEmpty()){
            return new ResponseEntity<>(pagamentos, HttpStatus.OK);
        } else {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
